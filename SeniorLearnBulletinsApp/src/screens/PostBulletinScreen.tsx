...
export default function PostBulletinScreen({
  ...
  const [bulletinType, setBulletinType] = useState<"member" | "official">(
    "member",
  );
  ...
  const handlePost = async () => {
    ...
    if (bulletinType === "official" && !canPostOfficial) {
      Alert.alert(
        "Error",
        "You do not have permission to post official bulletins, admin only.",
      );
      return;
    }
    if (!user?.token) {
    ...
    try {
      ...
      if (response.ok) {
        Alert.alert("Success", "Your bulletin has been posted successfully!", [
          {
            text: "OK",
            onPress: () => {
              setTitle("");
        ...
        Alert.alert(
          "Error",
          data.message || "Failed to post bulletin. Please try again.",
        );
      }
    } catch (error) {
              ...
              <View style={styles.radioContainer}>
                ...
                >
                  <View
                    style={[
                      styles.radioCircle,
                      bulletinType === "member" && styles.radioSelected,
                    ]}
                  >
                    {bulletinType === "member" && (
                  ...
                  <Text style={[styles.radioText, settingsStyle]}>
                    Member Bulletin
                  </Text>
                </TouchableOpacity>
              </View>{" "}
              ...
              <View style={styles.radioContainer}>
                ...
                >
                  <View
                    style={[
                      styles.radioCircle,
                      bulletinType === "official" && styles.radioSelected,
                    ]}
                  >
                    {bulletinType === "official" && (
                      <View style={styles.selectedDot} />
                    )}
                  </View>
                  <Text
                    style={[
                      styles.radioText,
                      settingsStyle,
                      canPostOfficial && styles.radioTextDisabled,
                    ]}
                  >
                    Official Bulletin {!canPostOfficial && "(Admin only)"}
                  </Text>
                </TouchableOpacity>
              </View>{" "}
              {/* end official type */}
            </View>{" "}
            {/* end bulletin type container */}
            {/* Bulletin fields */}
